...
def create_day_night_crashes_plot(df, selected_year=None, selected_route=None):
    """Create day vs night crash variations - TOP 10 SEGMENTS with horizontal bars."""
    filtered_df = df.copy()
    if selected_year and selected_year != "All Years":
        filtered_df = filtered_df[filtered_df['Year Of Crash'] == int(selected_year)]
    if selected_route and selected_route != "All Routes":
    ...
    # Get day and night columns - MORE FLEXIBLE matching
    daylight_cols = [col for col in pivot_data.columns if any(term in str(col).lower() for term in ['day', 'light', 'dawn', 'dusk', 'daylight'])]
    dark_cols = [col for col in pivot_data.columns if any(term in str(col).lower() for term in ['dark', 'night'])]
    
    # If no matches found, try a different approach
    if not daylight_cols and not dark_cols:
        all_cols = pivot_data.columns.tolist()
        dark_cols = [col for col in all_cols if 'dark' in str(col).lower() or 'night' in str(col).lower()]
        daylight_cols = [col for col in all_cols if col not in dark_cols]
    ...
    if day_crashes.sum() == 0 and night_crashes.sum() == 0:
        fig = go.Figure()
        fig.add_annotation(
            text=f"No crash data available for the selected filters.",
            xref="paper", yref="paper",
            x=0.5, y=0.5, showarrow=False,
    ...
    # Sort by total crashes (descending) to show most important segments first
    total_crashes = day_crashes + night_crashes
    sort_idx = total_crashes.sort_values(ascending=False).index
    day_crashes = day_crashes.loc[sort_idx]
    night_crashes = night_crashes.loc[sort_idx]
    
    segments = [str(seg) for seg in sort_idx.tolist()]
    
    # ============================================================
    # TOP 10 SEGMENTS ONLY
    # ============================================================
    top_n = 10
    segments = segments[:top_n]
    day_crashes = day_crashes.iloc[:top_n]
    night_crashes = night_crashes.iloc[:top_n]
    
    fig = go.Figure()
    
    # Day crashes - HORIZONTAL bars
    fig.add_trace(go.Bar(
        y=segments,
        x=day_crashes.values,
        name='Daytime',
        orientation='h',
        marker=dict(color='#ffa500', line=dict(color='black', width=1.2)),
        text=[f"{int(v):,}" for v in day_crashes.values],
        textposition='outside',
        textfont=dict(size=13, color='black', family='Arial Black'),
        hovertemplate='<b>%{y}</b><br>Daytime Crashes: %{x:,.0f}<extra></extra>'
    ))
    
    # Night crashes - HORIZONTAL bars
    fig.add_trace(go.Bar(
        y=segments,
        x=night_crashes.values,
        name='Nighttime',
        orientation='h',
        marker=dict(color='#191970', line=dict(color='black', width=1.2)),
        text=[f"{int(v):,}" for v in night_crashes.values],
        textposition='outside',
        textfont=dict(size=13, color='black', family='Arial Black'),
    ...
    fig.update_layout(
        title={
            'text': '<b>✨ Top 10 Segments: Day vs Night Crash Comparison</b>',  # ← Updated title
            'font': {'size': 24, 'family': 'Arial', 'color': '#1f77b4'},
            'x': 0.5,
            'xanchor': 'center'
        },
        xaxis_title={'text': '<b>Number of Crashes</b>', 'font': {'size': 20, 'family': 'Arial', 'color': 'black'}},
        yaxis_title={'text': '<b>Segment ID (Ranked by Total Crashes)</b>', 'font': {'size': 20, 'family': 'Arial', 'color': 'black'}},
        height=650,  # ← Fixed height for top 10
        template='plotly_white',
        paper_bgcolor='white',
        plot_bgcolor='white',
        barmode='group',
        ...
        yaxis=dict(
            ...
            linewidth=2,
            mirror=True,
            autorange='reversed'  # Top to bottom ordering (Rank 1 at top)
        ),
...
def create_segment_ranking_plots(df, selected_year=None, selected_route=None):
    """Create two ranking plots with gradient colors and thousand separators."""
    ...
    # Create gradient colors for blue (dark to light)
    # RGB for dark blue: (0, 51, 153) to light blue: (173, 216, 230)
    n_bars = len(crash_counts)
    blue_gradient = [
        f'rgb({int(0 + (173-0)*i/(n_bars-1))}, '
        f'{int(51 + (216-51)*i/(n_bars-1))}, '
        f'{int(153 + (230-153)*i/(n_bars-1))})'
    ...
    fig1.add_trace(go.Bar(
        ...
        hovertemplate='<b>Rank: %{x}</b><br>Segment: %{customdata[0]}<br>Route: %{customdata[1]}<br>Total Crashes: %{y:,.0f}<extra></extra>',
    ...
    # Create gradient colors for red (dark to light)
    # RGB for dark red: (139, 0, 0) to light red: (255, 182, 193)
    n_bars_red = len(hit_run_counts)
    red_gradient = [
        f'rgb({int(139 + (255-139)*i/(n_bars_red-1))}, '
        f'{int(0 + (182-0)*i/(n_bars_red-1))}, '
        f'{int(0 + (193-0)*i/(n_bars_red-1))})'
        for i in range(n_bars_red)
    ]
    ...
    fig2.add_trace(go.Bar(
        ...
        textposition='outside',
        textfont=dict(size=14, color='black', family='Arial Black'),
        hovertemplate='<b>Rank: %{x}</b><br>Segment: %{customdata[0]}<br>Route: %{customdata[1]}<br>Hit & Run Cases: %{y:,.0f}<extra></extra>',
...
def create_fatality_ranking_plot(df, selected_year=None, selected_route=None):
    """Create top 10 segments by total fatalities — used on Page 01 only."""
    ...
    # ========== Fatalities ranking ==========
    fatality_counts = filtered_df.groupby(['Segment ID', 'Route']).agg(
        Total_Fatalities=('fatalities', 'sum')  # works if col is 'fatalities'; see rename note below
    ).reset_index()
    
    # fallback: if the column is named differently after rename in show_hotspot_maps_page
    if 'Total_Fatalities' not in fatality_counts.columns or fatality_counts['Total_Fatalities'].sum() == 0:
        # Try common alternative column names present in analysis_df
        for col_candidate in ['Total Kill', 'fatalities', 'Fatalities']:
            if col_candidate in filtered_df.columns:
                fatality_counts = filtered_df.groupby(['Segment ID', 'Route'])[col_candidate].sum().reset_index()
                fatality_counts = fatality_counts.rename(columns={col_candidate: 'Total_Fatalities'})
                break
    
    fatality_counts = fatality_counts.sort_values('Total_Fatalities', ascending=False).head(10)
    fatality_counts = fatality_counts[fatality_counts['Total_Fatalities'] > 0]  # drop zeros
    fatality_counts['Rank'] = range(1, len(fatality_counts) + 1)
    
    # Red gradient: dark red (139,0,0) → light red (255,182,193)
    n_bars = len(fatality_counts)
    if n_bars > 1:
        red_gradient = [
            f'rgb({int(139 + (255-139)*i/(n_bars-1))}, '
            f'{int(0 + (182-0)*i/(n_bars-1))}, '
            f'{int(0 + (193-0)*i/(n_bars-1))})'
            for i in range(n_bars)
        ]
    else:
        red_gradient = ['rgb(139,0,0)']
    fatality_counts['Color'] = red_gradient
    
    fig = go.Figure()
    
    fig.add_trace(go.Bar(
        x=fatality_counts['Rank'],
        y=fatality_counts['Total_Fatalities'],
        marker=dict(
            color=fatality_counts['Color'],
            line=dict(color='black', width=2)
        ),
        width=0.6,
        text=[f"<b>{seg}</b><br>{int(count):,} fatalities"
              for seg, count in zip(fatality_counts['Segment ID'], fatality_counts['Total_Fatalities'])],
        textposition='outside',
        textfont=dict(size=14, color='black', family='Arial Black'),
        hovertemplate='<b>Rank: %{x}</b><br>Segment: %{customdata[0]}<br>Route: %{customdata[1]}<br>Total Fatalities: %{y:,.0f}<extra></extra>',
        customdata=fatality_counts[['Segment ID', 'Route']].values
    ))
    
    fig.update_layout(
        title={
            'text': '<b>✨ Top 10 Segments by Fatalities</b>',
            'font': {'size': 24, 'family': 'Arial', 'color': '#dc3545'},
            'x': 0.5,
            'xanchor': 'center',
            'y': 0.98,
        ...
        xaxis_title={
            'text': '<b>Rank (1 = Highest Fatalities)</b>',
            'font': {'size': 19, 'family': 'Arial', 'color': 'black'}
        },
        yaxis_title={
            'text': '<b>Number of Fatalities</b>',
            'font': {'size': 19, 'family': 'Arial', 'color': 'black'}
        },
        height=650,
        template='plotly_white',
...
def load_crash_shapefile_data():
    ...
    try:
        ...
        # Only read necessary columns - ADD 'Date of Cr' here
        usecols = [
            'GPS Coordi', 'GPS Coor_1', 'MSLINK', 'Type of Cr', 
            'Year Of Cr', 'Total Kill', 'Total Inj', 
            'Hit and Ru', 'RTE_NME', 'CNTY_SEAT', 'Date of Cr', 'Light Cond'  # ADDED THESE TWO
        ]
        ...
        df = pd.read_csv(
            ...
            dtype={
                ...
                'CNTY_SEAT': 'category',
                'Hit and Ru': 'category',
                'Light Cond': 'category'  # ADDED THIS
            },
            low_memory=False
        )
        
        # Rename columns - ADD THESE TWO MAPPINGS
        df = df.rename(columns={
            ...
            'RTE_NME': 'route',
            'CNTY_SEAT': 'city',
            'Date of Cr': 'Date of Crash',  # ADDED THIS
            'Light Cond': 'Light Condition'  # ADDED THIS
        })
...
def create_severity_scatter_map(df, selected_year=None, selected_route=None):
    """Create scatter map colored by crash severity with improved visibility"""
    ...
    # Define severity categories and colors - CUSTOM VIBRANT COLORS
    # severity_colors = {
    #     'Fatal Injury': '#8B0000',                    # Dark Red
    #     'Incapacitating Injury': '#DC143C',           # Crimson Red
    #     'Non-Incapacitating Injury': '#FF6347',       # Tomato Red
    #     'Possible Injury': '#FFA500',                 # Orange
    #     'Property Damage Only': '#FFD700'             # Gold
    # }
    
    # ALTERNATIVE COLOR SCHEME (Uncomment if you prefer distinct colors):
    severity_colors = {
        'Fatal Injury': '#96092B',              # Dark Red
        'Incapacitating Injury': '#FF4500',     # Orange Red
        'Non-Incapacitating Injury': '#FFD700', # Gold
        'Possible Injury': '#32CD32',           # Lime Green
        'Property Damage Only': '#1E90FF'       # Dodger Blue
    }
    ...
    # ============================================================
    # FIX: Make all markers UNIFORM SIZE (not based on fatalities)
    # ============================================================
    filtered_df['marker_size'] = 10  # ← All same size
    
    # Optional: Slightly larger size for fatal crashes
    # filtered_df['marker_size'] = filtered_df['severity'].apply(
    #     lambda x: 15 if x == 'Fatal Injury' else 10
    # )
    ...
    fig = px.scatter_mapbox(
        ...
        size='marker_size',  # ← Changed from 'fatalities' to 'marker_size'
        size_max=10,  # ← Uniform maximum size
        ...
        hover_data={
            ...
            'injuries': True,
            'year': True,
            'marker_size': False  # ← Hide this from hover
        },
        center=dict(lat=35.15, lon=-90.05),
        zoom=9,
    ...
    # ============================================================
    # REMOVED: The update_traces section that caused the error
    # Scattermapbox doesn't support marker line borders
    # ============================================================
...
def show_hotspot_maps_page():
    ...
    # ADD THIS: Show a button to load data instead of loading immediately
    ...
    if not st.session_state.crash_data_loaded:
        ...
        if st.button("🔅 Load Crash Data", use_container_width=True):
            with st.spinner("Loading crash data... This may take a moment..."):
    ...
    if crash_df is None or crash_df.empty:
        ...
        st.info("Expected file: `data/Unlocked_Segmented_2022_2025_Crashes_Final.csv`")
        return
    ...
    with col2:
        st.metric("🔅 Total Fatalities", f"{int(crash_df['fatalities'].sum()):,}")
    with col3:
        st.metric("🔅 Total Injuries", f"{int(crash_df['injuries'].sum()):,}")
    with col4:
        unique_segments = crash_df['segment_id'].nunique()
        st.metric("🔅 Unique Segments", f"{unique_segments:,}")
    ...
    # ========== NEW SECTION: ADDITIONAL ANALYSIS ==========
    st.markdown("### 🔅 Historical Background of Crash Data Analysis")
    
    # Prepare data for analysis plots - convert to match format expected by existing functions
    analysis_df = crash_df.copy()
    ...
    fig_monthly = create_monthly_crashes_plot(analysis_df, selected_year, selected_route, None)
    st.plotly_chart(fig_monthly, use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)
    
    st.markdown("<br>", unsafe_allow_html=True)
    
    # Day vs Night crashes plot
    st.markdown("#### 🔅 Day vs Night Crash Comparison")
    st.markdown("""
    <div style="border: 1px solid #1f77b4; border-radius: 10px; padding: 15px; background-color: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    """, unsafe_allow_html=True)
    
    fig_daynight = create_day_night_crashes_plot(analysis_df, selected_year, selected_route)
    ...
    fig_crash, _ = create_segment_ranking_plots(analysis_df, selected_year, selected_route)  # only reuse total-crashes plot
    fig_fatalities = create_fatality_ranking_plot(analysis_df, selected_year, selected_route)
