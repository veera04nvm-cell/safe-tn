...
def login_page():
    ...
    with col2:
        st.image(r"D:\OneDrive - The University of Memphis\2024_THSO_DUI\Dashboard\images\Speeding_Crashes.jpg", width=100)
...
def load_data():
    ...
    try:
        weekly_df = pd.read_csv(r'D:\OneDrive - The University of Memphis\2024_THSO_DUI\Dashboard\data\weekly_crashes_enhanced.csv')
        weekly_df['week_start'] = pd.to_datetime(weekly_df['week_start'])
        
        future_df = pd.read_csv(r'D:\OneDrive - The University of Memphis\2024_THSO_DUI\Dashboard\data\future_predictions_ml.csv')
        future_df['week_start'] = pd.to_datetime(future_df['week_start'])
        
        metrics_df = pd.read_csv(r'D:\OneDrive - The University of Memphis\2024_THSO_DUI\Dashboard\data\model_performance_comparison.csv')
...
def create_forecast_plot(weekly_df, future_df, show_risk_zones=True):
    ...
    fig.update_layout(
        title='Weekly Crash History & 12-Week Forecast',
        xaxis_title='Date',
...
def create_risk_distribution(future_df):
    ...
    fig.update_layout(
        title='Risk Level Distribution (Next 12 Weeks)',
        height=400,
        template='plotly_white'
    )
...
def main():
    ...
    if page == "🏠 Dashboard Overview":
        # Header
        st.title("🚗 Traffic Crash Prediction Dashboard")
        ...
        with col1:
            st.metric(
                label="📊 Total Expected (12 weeks)",
                value=f"{total_predicted:.1f}",
                delta=f"{((total_predicted/12 - weekly_df['total_crashes'].mean()) / weekly_df['total_crashes'].mean() * 100):.1f}% vs avg"
            )
        ...
        # Main Forecast Plot
        st.subheader("📊 Historical Data & 12-Week Forecast")
    ...
    # ========================================================================
        ...
        st.markdown("### Next 12 weeks of crash predictions")
    ...
    # ========================================================================
        ...
        with st.expander("Can I trust predictions far into the future?"):
            st.markdown("""
            - **Near-term (1-4 weeks)**: Most reliable
            - **Medium-term (5-8 weeks)**: Good for planning
            - **Longer-term (9-12 weeks)**: General trends only
            
            The model assumes normal conditions. Unexpected events (major construction, special events, weather) can change outcomes.
            """)
